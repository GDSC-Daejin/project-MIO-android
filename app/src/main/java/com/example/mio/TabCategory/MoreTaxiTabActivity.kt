        ...
        myViewModel.checkSearchFilter.observe(this) {
            when(it) {
                ...
                "마감 임박 순" -> {
                    ...
                    moreTaxiAllData.sortedWith(compareBy<PostData?> { sdf.parse(it?.postTargetDate + " " + it?.postTargetTime) }
    ...
    private fun getMoreItem() {
        ...
        handler.postDelayed({
            ...
            if (currentPage < totalPages - 1) {
                currentPage += 1
                CoroutineScope(Dispatchers.IO).launch {
                    RetrofitServerConnect.create(this@MoreTaxiTabActivity).getCategoryPostData(2,"createDate,desc", currentPage, 5).enqueue(object : Callback<PostReadAllResponse> {
